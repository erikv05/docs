...
### Using Metadata with Generate Football Highlights

The metadata JSON from this endpoint can be passed directly to the [Generate Football Highlights](/api-reference/clipping/generate-highlights) endpoint for enhanced accuracy. When metadata is provided, highlight generation uses:
...
# Step 3: Generate Football Highlights with metadata
curl -X POST https://api.usenarrative.ai/api/v1/highlights/generate \
  -F "video=@game.mp4" \
  -F "metadata=@metadata.json"
```

<Card title="Generate Football Highlights" icon="wand-magic-sparkles" href="/api-reference/clipping/generate-highlights">
